...
## PS1 CONFIG {{{
  ...
  if $_isxrunning; then
    ...
    if [[ $PS1 && -f /usr/share/git/git-prompt.sh ]]; then
      ...
      export PS1="$GY[$B\u$GY@$G\h$GY:$B\W\$(__git_ps1 \"$GY|$LB%s\")$GY]$W\$(get_prompt_symbol) "
    else
      export PS1="$GY[$B\u$GY@$G\h$GY:$B\W$GY]$W\$(get_prompt_symbol) "
    fi
  else
...
## BASH OPTIONS {{{
  ...
  #}}}
#}}}
# ignore bash items starting with a space
...
## ALIAS {{{
  ...
  # PACMAN ALIASES {{{
    ...
    if $_isarch; then
      ...
      alias pacrm='pacman -Rs $(pacman -Qtdq)' # Remove all packages that are unneeded.
